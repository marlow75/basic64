...
public class MOS6502Test {
	...
	public void resetTest() {
		...
		while (MOS6502.B != 16) // check if BRK instruction
			MOS6502.executeNext(); // execute next instruction
	}
...
//        adc ($fb),y
	...
	public void SBC() {
		...
		assert (MOS6502.AC == 151);
		assert (MOS6502.C == 0); // borrow
		assert (MOS6502.V == 0);
		...
		assert (MOS6502.C == 0);
		assert (MOS6502.V == 0);
		assert (MOS6502.AC == 121);

		// sbc #50 (45-50) BCD
		MOS6502.PC = 0x1000;

		MOS6502.C = 0; // borrowed
		MOS6502.D = 8;
		...
		MOS6502.executeNext();
		assert (MOS6502.AC == 0x94);
		assert (MOS6502.C == 0); // borrowed		
...
//        brk   
		...
		for (int i = 0; i < 255; i++) {
			...
			data++;
			assert (MOS6502.X == MOS6502.Y);
			assert (MOS6502.X == (data & 0xff));
		}
	}
...
//        bne @loop
		...
		assert (MOS6502.PC == 0x1000);
		Memory.fill(0xd012, new int[] { 0 }); // raster = 0

		MOS6502.executeNext();
		MOS6502.executeNext();

		assert (MOS6502.PC == 0x1005);
	}
...
//		FD2D   10 F1      BPL $FD20
	...
	public void CNZFlags() {
		for (int a = 0; a < 256; a++)
			for (int b = 0; b < 256; b++) {
				...
				MOS6502.sub(a, b); // without carry CMP, CPY, CPX

				if (a < b)
					assert  MOS6502.C == 0 : a + " " + b;

				if (a > b)
					assert MOS6502.C == 1 : a + " " + b;

				if (a == b)
					assert  MOS6502.Z == 2 : a + " " + b;
				else
					assert  MOS6502.Z == 0: a + " " + b;
			}
	}
	...
	public void basicFree() {
		...
		assert MOS6502.AC * 256 + MOS6502.X == 38911; 
	}
